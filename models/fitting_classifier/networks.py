...
__all__ = ['DenseNet', 'densenet121', 'densenet169', 'densenet201', 'densenet161', 'densenet201_multi']
...
def densenet201_multi(nchannels=3, **kwargs):
    model = DenseNetMulti(nchannels, num_init_features=64, growth_rate=32, block_config=(6, 12, 48, 32),
                     **kwargs)
    # if pretrained: model.load_state_dict(model_zoo.load_url(model_urls['densenet201']))
    return model    
...
class DenseNetMulti(nn.Module):
    ...
    Args:
        ...
        num_classes (int) - number of classification classes
    """
    def __init__(self, nchannels=3, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_classes=2):

        super(DenseNetMulti, self).__init__()

        # First convolution
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(nchannels, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)),
            ('norm0', nn.BatchNorm2d(num_init_features)),
            ('relu0', nn.ReLU(inplace=True)),
            ('pool0', nn.MaxPool2d(kernel_size=3, stride=2, padding=1)),
        ]))
    ...
    def initialize(self, opt): #, which_model_netC='densenet', init_type='normal', gpu_ids=[]):

        # setting the last layer
        num_ftrs = self.classifier.in_features
        print('Classifier in features: ', num_ftrs)
        ...
        if opt.checkpoint: 
                # be sure there is only one .pth file in the checkpoint folder
                model_path = glob(os.path.join(os.path.join(opt.checkpoint_dir, opt.checkpoint), '*.pth'))[-1]
